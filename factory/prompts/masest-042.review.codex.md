Review Loop Factory spec `masest-042` against current working tree.

        Native adapter: `codex`
        Spec path: `/Users/omar/Claude/Projects/MASEST/factory/specs/active/masest-042.md`
...
- Do not run `npm test`: its glob includes Playwright files. Broad gate: `files=(); for f in tests/*.test.mjs; do grep -Eqi "playwright|@playwright" "$f" && continue; [[ "${f:t}" == "cart-page.test.mjs" ]] && continue; files+=("$f"); done; node --test --test-concurrency=1 --test-timeout=120000 "${files[@]}"`.
