---
id: masest-029
title: Add a durable ledger for Stripe webhook effects
agent: codex
risk: high
grill: completed
verification:
...
- Accepted on 2026-07-19 after independent review of the isolated staging replay/crash proof. Row 029 is `DONE`.
- The staging runtime used Supabase project `qmvzytwdsgfuysgosbhj` and Cloudflare Pages project `masest-commerce-staging`; production remained on distinct project ref `mvfxzvkzcqmnwcoblvfc`.
- A signed dispute event replay produced exactly one durable effect. The worker reclaimed it after an intentional post-provider crash and completed it on attempt 2; replaying the stable provider idempotency key returned the original provider message ID.
