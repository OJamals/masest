...
//   GET ?status=&limit=     → { orders: [...] } across all companies (with company name + items)
//   POST { id, status }     → update an order's status (+ notify the company)
...
export default async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    // Notify the buyer's company of the status change.
    if (order?.company_id) {
