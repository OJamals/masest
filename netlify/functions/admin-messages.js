...
//   GET                       → { threads: [...] } one row per company that has messages (+ unread)
//   GET ?company_id=<id>      → { messages: [...] } full thread; marks buyer msgs read_by_staff
//   POST { company_id, body } → staff reply (+ notify the company)
...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    // Thread list: aggregate in JS (bounded). Newest activity first.
    const { data, error } = await sb
      .from('messages')
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await sb.from('messages').insert({
      company_id: companyId, user_id: null, sender_role: 'staff',
      body: text, read_by_staff: true, read_by_user: false,
    }).select('id,created_at').single();
    if (error) return json(500, { error: error.message });

    await sb.from('notifications').insert({
      company_id: companyId, type: 'message',
      title: 'New message from MASEST', body: text.slice(0, 140),
...
export const config = { path: '/api/admin/messages' };
