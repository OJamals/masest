...
test("marine catalog loads the public artifact with a source-tree fallback", async () => {
  ...
  const entries = await loadMarineCatalog(async (url) => {
    requested.push(url);
    if (url === "/data/marine-catalog.json?v=20260908b") return { ok: false };
    return { ok: true, async json() { return payload; } };
  });

  assert.deepEqual(requested, ["/data/marine-catalog.json?v=20260908b", "/data/industry-applications.json"]);
  assert.equal(entries.length, 8);
});
