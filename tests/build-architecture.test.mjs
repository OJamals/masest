...
test("Cloudflare build emits baseline security headers", () => {
  ...
  assert.match(build, /font-src[^;]*https:\/\/client\.crisp\.chat/, "Crisp widget fonts must be allowed or live chat logs CSP errors");
...
test("HTML pages use one fresh shared stylesheet cache-buster", () => {
  ...
  assert.deepEqual([...versions], ["20260706b"]);
});
