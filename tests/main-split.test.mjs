...
test("changed public module graph shares one cache release", () => {
  ...
  assert.match(productSearch, new RegExp(`fuzzy-search\\.js\\?v=${release}`));
  assert.match(serviceCatalog, new RegExp(`fuzzy-search\\.js\\?v=${release}`));
...
test("main entrypoint imports chrome rendering from a split module", () => {
  const main = read("js/main.js");
  assert.match(main, /from\s+["']\.\/main\/chrome\.js(?:\?v=\d{8}[a-z])?["']/);
