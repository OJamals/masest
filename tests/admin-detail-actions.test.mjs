...
test("company detail panel exposes direct action buttons", () => {
  ...
  assert.match(src, /data-company-support-thread/, "detail panel should open the company support thread");
  assert.match(src, /openSupportThread\?\.\(company\.id\)/, "support action should target the selected company");
