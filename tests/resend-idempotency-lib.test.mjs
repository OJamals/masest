...
test('sendEmailResult accepts idempotency/reply/text/header/attachment options and sendEmail preserves boolean compatibility', () => {
...
test('Idempotency-Key header is stable, collision-safe, and set only when supplied', () => {
  ...
  assert.match(src, /if \(idempotencyKey\) requestHeaders\['Idempotency-Key'\] = await providerIdempotencyHeader\(idempotencyKey\)/);
  assert.match(src, /headers: requestHeaders/);
});

test('RFC thread and unsubscribe headers are Resend message headers, not HTTP headers', () => {
  assert.match(src, /const messageHeaders = Object\.fromEntries/);
  assert.match(src, /messageHeaders\['List-Unsubscribe'\]/);
  assert.match(src, /Object\.keys\(messageHeaders\)\.length \? \{ headers: messageHeaders \} : \{\}/);
  assert.doesNotMatch(src, /requestHeaders\['List-Unsubscribe'\]/);
});
