...
const SUPPORT_DELIVERY = readFileSync(new URL("../functions/_lib/support-delivery.js", import.meta.url), "utf8");
const SUPPORT_EMAIL_DELIVERY = readFileSync(new URL("../functions/_lib/support-email-delivery.js", import.meta.url), "utf8");
...
test("buyer POST records chat presence and uses shared support email delivery", () => {
  ...
  assert.match(SUPPORT_PUBLISHER, /attemptSupportMessageDelivery/);
  assert.doesNotMatch(SUPPORT_PUBLISHER, /deliverSupportMessageEmail/);
  assert.match(SUPPORT_DELIVERY, /processClaimedIntegrationEffect/);
  assert.match(ACCOUNT, /action === 'chat_presence'/);
  assert.match(ACCOUNT, /support_chat_open/);
  assert.match(SUPPORT_EMAIL, /attemptSupportMessageDelivery/);
  assert.match(SUPPORT_EMAIL_DELIVERY, /adminMessageAlertKind/);
  assert.match(SUPPORT_EMAIL_DELIVERY, /adminMessageRecipients/);
});
...
test("staff messages target a company user and shared delivery honors buyer presence", () => {
  ...
  assert.match(SUPPORT_EMAIL_DELIVERY, /shouldEmailSupportRecipient/);
  assert.match(SUPPORT_MESSAGES, /support_chat_open/);
  assert.match(SUPPORT_MESSAGES, /emailsByIds/);
});
