...
export async function initCustomerChat() {
  ...
  let authModule;
  const auth = async () => {
    authModule ||= import(window.MASEST?.authModule || "./auth.js?v=20260915a");
    return authModule;
  };
  try {
    ...
    if (await session.getToken()) {
      ...
      if (account?.can_admin) {
        ...
        const { initAdminSupport } = await import("./admin-support.js?v=20260915a");
  ...
  if (!document.querySelector('link[data-masest-customer-chat="true"]')) {
    ...
    stylesheet.href = `${root}css/customer-chat.css?v=20260915a`;
  ...
  const updateQuoteHref = async () => {
    try {
      [requestContextModule, cartModule] = await Promise.all([
        requestContextModule || import("./request-context.js?v=20260915a"),
        cartModule || import("./cart.js?v=20260915a"),
      ]);
