...
import { esc, delegate, confirmDialog, restoreFocusOnClose } from '../util.js?v=20260912a';
import {
  createRichTextEditor,
  referencePickerTemplate,
  refreshRichTextEditor,
  richEditorTemplate,
} from './rich-editor.js?v=20260912a';
import { renderNewsletterBody } from '../newsletter-render.js?v=20260912a';
import { openImageLibraryPicker } from './image-library-picker.js?v=20260912a';
...
export function recipientSourceLabel(value) {
...
export function createNewsletterTab({ $, api, state, message, admSkeleton, admEmpty, badge }) {
  ...
  let counts = { subscribers: 0, imported: 0 };
  ...
  async function loadCounts() {
    try {
      const res = await api('/api/admin/recipients');
      counts = res.counts || { subscribers: 0, imported: 0 };
    } catch { /* best-effort — campaign composer still works without live counts */ }
    badge('nlCountSubscribers', counts.subscribers || 0);
    updateAudienceEstimate();
  }
