...
import { login, logout, api, getToken } from './auth.js?v=20260721a';
import { esc, safeUrl, money, wireTablist, rovingTabindex, linkTabsToPanels } from './util.js?v=20260721a';
import { editKey } from './admin/edits.js?v=20260721a';
import { createFeatureLoader } from './admin/feature-loader.js?v=20260721a';
import { applyCapabilityUi, normalizeStaffContext, staffRoleLabel } from './admin/permissions.js?v=20260721a';
import { renderChrome } from './main/chrome.js?v=20260721a';
...
const featureLoader = createFeatureLoader({
  analytics: async () => {
    const [{ createTrafficRenderer }, { createSeoAudit }] = await Promise.all([
      import('./admin/traffic.js?v=20260721a'),
      import('./admin/seo.js?v=20260721a'),
    ]);
  ...
  integrations: async () => {
    const { connectQbo, disconnectQbo, renderQboStatus, runQboSync } = await import('./admin/qbo.js?v=20260721a');
    return {
      wire() {
  ...
  orders: async () => {
    const { ORDER_STATUSES, createOrdersTab } = await import('./admin/orders.js?v=20260721a');
  ...
  companies: async () => {
    const [{ createCompaniesTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/companies.js?v=20260721a'),
      import('./admin/crm.js?v=20260721a'),
    ]);
  ...
  products: async () => {
    const [
      ...
      import('./admin/products.js?v=20260721a'),
      import('./admin/pricing.js?v=20260721a'),
      import('./admin/inventory.js?v=20260721a'),
      import('./admin/coupons.js?v=20260721a'),
    ]);
  ...
  support: async () => {
    const { createThreadsTab } = await import('./admin/threads.js?v=20260721a');
  ...
  quotes: async () => {
    const { createQuotesTab } = await import('./admin/quotes.js?v=20260721a');
  ...
  reviews: async () => {
    const { createReviewsTab } = await import('./admin/reviews.js?v=20260721a');
    const {
      renderReviews,
      wireReviews,
  ...
  crm: async () => {
    const [{ createCrmWorkspace }, { createOffersTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/crm-workspace.js?v=20260721a'),
      import('./admin/offers.js?v=20260721a'),
      import('./admin/crm.js?v=20260721a'),
    ]);
    const crm = createCrmPanel({ $, api, admSkeleton, admEmpty });
