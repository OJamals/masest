...
import { login, logout, api, apiBlob, getToken } from './auth.js?v=20260902d';
import { esc, safeUrl, money, wireTablist, rovingTabindex, linkTabsToPanels, delegate, confirmDialog } from './util.js?v=20260902d';
import { editKey } from './admin/edits.js?v=20260902d';
import { createFeatureLoader } from './admin/feature-loader.js?v=20260902d';
import { applyCapabilityUi, normalizeStaffContext, staffRoleLabel } from './admin/permissions.js?v=20260902d';
import { renderAdminChrome, setAdminChromeSession } from './admin/chrome.js?v=20260902d';
import { createAdminSessionLifecycle } from './admin/session.js?v=20260902d';
import { createAdminSearch } from './admin/search.js?v=20260902d';
...
function wireReports() {
  void import('./admin/stripe.js?v=20260902d').then(({ wireStripePayouts, renderStripePayouts }) => {
    wireStripePayouts();
...
const featureLoader = createFeatureLoader({
  analytics: async () => {
    const [{ createTrafficRenderer }, { createSeoAudit }] = await Promise.all([
      import('./admin/traffic.js?v=20260902d'),
      import('./admin/seo.js?v=20260902d'),
    ]);
  ...
  integrations: async () => {
    const { connectQbo, disconnectQbo, renderQboStatus, runQboSync } = await import('./admin/qbo.js?v=20260902d');
    const { renderShipStationStatus, wireShipStationStatus } = await import('./admin/shipstation.js?v=20260902d');
    const { renderStripeStatus } = await import('./admin/stripe.js?v=20260902d');
    const { renderIntegrationHealth, wireIntegrationHealth } = await import('./admin/integration-health.js?v=20260902d');
    const { createAutomationCard } = await import('./admin/automation.js?v=20260902d');
  ...
  orders: async () => {
    const { ORDER_STATUSES, NEEDS_FULFILLMENT, createOrdersTab } = await import('./admin/orders.js?v=20260902d');
  ...
  companies: async () => {
    const [{ createCompaniesTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/companies.js?v=20260902d'),
      import('./admin/crm.js?v=20260902d'),
    ]);
  ...
  products: async () => {
    const [
      ...
      import('./admin/products.js?v=20260902d'),
      import('./admin/pricing.js?v=20260902d'),
      import('./admin/inventory.js?v=20260902d'),
      import('./admin/coupons.js?v=20260902d'),
    ]);
  ...
  content: async () => {
    const { createContentTab } = await import('./admin/content.js?v=20260902d');
  ...
  support: async () => {
    const { createThreadsTab } = await import('./admin/threads.js?v=20260902d');
  ...
  quotes: async () => {
    const { createQuotesTab } = await import('./admin/quotes.js?v=20260902d');
  ...
  reviews: async () => {
    const { createReviewsTab } = await import('./admin/reviews.js?v=20260902d');
    const {
      renderReviews,
      wireReviews,
  ...
  newsletter: async () => {
    const [{ createNewsletterTab }, { createOffersTab }] = await Promise.all([
      import('./admin/newsletter.js?v=20260902d'),
      import('./admin/offers.js?v=20260902d'),
    ]);
  ...
  crm: async () => {
    const [{ createCrmWorkspace }, { createCrmPanel }] = await Promise.all([
      import('./admin/crm-workspace.js?v=20260902d'),
      import('./admin/crm.js?v=20260902d'),
    ]);
    const crm = createCrmPanel({ $, api, admSkeleton, admEmpty });
