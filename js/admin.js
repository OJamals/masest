...
import { login, logout, api, getToken } from './auth.js?v=20260724b';
import { esc, safeUrl, money, wireTablist, rovingTabindex, linkTabsToPanels } from './util.js?v=20260724b';
import { editKey } from './admin/edits.js?v=20260724b';
import { createFeatureLoader } from './admin/feature-loader.js?v=20260724b';
import { applyCapabilityUi, normalizeStaffContext, staffRoleLabel } from './admin/permissions.js?v=20260724b';
import { renderChrome } from './main/chrome.js?v=20260724b';
...
const featureLoader = createFeatureLoader({
  analytics: async () => {
    const [{ createTrafficRenderer }, { createSeoAudit }] = await Promise.all([
      import('./admin/traffic.js?v=20260724b'),
      import('./admin/seo.js?v=20260724b'),
    ]);
  ...
  integrations: async () => {
    const { connectQbo, disconnectQbo, renderQboStatus, runQboSync } = await import('./admin/qbo.js?v=20260724b');
    return {
      wire() {
  ...
  orders: async () => {
    const { ORDER_STATUSES, createOrdersTab } = await import('./admin/orders.js?v=20260724b');
  ...
  companies: async () => {
    const [{ createCompaniesTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/companies.js?v=20260724b'),
      import('./admin/crm.js?v=20260724b'),
    ]);
  ...
  products: async () => {
    const [
      ...
      import('./admin/products.js?v=20260724b'),
      import('./admin/pricing.js?v=20260724b'),
      import('./admin/inventory.js?v=20260724b'),
      import('./admin/coupons.js?v=20260724b'),
    ]);
  ...
  content: async () => {
    const { createContentTab } = await import('./admin/content.js?v=20260724b');
  ...
  support: async () => {
    const { createThreadsTab } = await import('./admin/threads.js?v=20260724b');
  ...
  quotes: async () => {
    const { createQuotesTab } = await import('./admin/quotes.js?v=20260724b');
  ...
  reviews: async () => {
    const { createReviewsTab } = await import('./admin/reviews.js?v=20260724b');
    const {
      renderReviews,
      wireReviews,
  ...
  newsletter: async () => {
    const { createNewsletterTab } = await import('./admin/newsletter.js?v=20260724b');
  ...
  crm: async () => {
    const [{ createCrmWorkspace }, { createOffersTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/crm-workspace.js?v=20260724b'),
      import('./admin/offers.js?v=20260724b'),
      import('./admin/crm.js?v=20260724b'),
    ]);
    const crm = createCrmPanel({ $, api, admSkeleton, admEmpty });
