...
import { login, logout, api, apiBlob, getToken } from './auth.js?v=20260830g';
import { esc, safeUrl, money, wireTablist, rovingTabindex, linkTabsToPanels, delegate, confirmDialog } from './util.js?v=20260830g';
import { editKey } from './admin/edits.js?v=20260830g';
import { createFeatureLoader } from './admin/feature-loader.js?v=20260830g';
import { applyCapabilityUi, normalizeStaffContext, staffRoleLabel } from './admin/permissions.js?v=20260830g';
import { renderAdminChrome, setAdminChromeSession } from './admin/chrome.js?v=20260830g';
import { createAdminSessionLifecycle } from './admin/session.js?v=20260830g';
import { createAdminSearch } from './admin/search.js?v=20260830g';
...
function wireReports() {
  void import('./admin/stripe.js?v=20260830g').then(({ wireStripePayouts, renderStripePayouts }) => {
    wireStripePayouts();
...
const featureLoader = createFeatureLoader({
  analytics: async () => {
    const [{ createTrafficRenderer }, { createSeoAudit }] = await Promise.all([
      import('./admin/traffic.js?v=20260830g'),
      import('./admin/seo.js?v=20260830g'),
    ]);
  ...
  integrations: async () => {
    const { connectQbo, disconnectQbo, renderQboStatus, runQboSync } = await import('./admin/qbo.js?v=20260830g');
    const { renderShipStationStatus, wireShipStationStatus } = await import('./admin/shipstation.js?v=20260830g');
    const { renderStripeStatus } = await import('./admin/stripe.js?v=20260830g');
    const { renderIntegrationHealth, wireIntegrationHealth } = await import('./admin/integration-health.js?v=20260830g');
    const { createAutomationCard } = await import('./admin/automation.js?v=20260830g');
  ...
  orders: async () => {
    const { ORDER_STATUSES, NEEDS_FULFILLMENT, createOrdersTab } = await import('./admin/orders.js?v=20260830g');
  ...
  companies: async () => {
    const [{ createCompaniesTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/companies.js?v=20260830g'),
      import('./admin/crm.js?v=20260830g'),
    ]);
  ...
  products: async () => {
    const [
      ...
      import('./admin/products.js?v=20260830g'),
      import('./admin/pricing.js?v=20260830g'),
      import('./admin/inventory.js?v=20260830g'),
      import('./admin/coupons.js?v=20260830g'),
    ]);
  ...
  content: async () => {
    const { createContentTab } = await import('./admin/content.js?v=20260830g');
  ...
  support: async () => {
    const { createThreadsTab } = await import('./admin/threads.js?v=20260830g');
  ...
  quotes: async () => {
    const { createQuotesTab } = await import('./admin/quotes.js?v=20260830g');
  ...
  reviews: async () => {
    const { createReviewsTab } = await import('./admin/reviews.js?v=20260830g');
    const {
      renderReviews,
      wireReviews,
  ...
  newsletter: async () => {
    const [{ createNewsletterTab }, { createOffersTab }] = await Promise.all([
      import('./admin/newsletter.js?v=20260830g'),
      import('./admin/offers.js?v=20260830g'),
    ]);
  ...
  crm: async () => {
    const [{ createCrmWorkspace }, { createCrmPanel }] = await Promise.all([
      import('./admin/crm-workspace.js?v=20260830g'),
      import('./admin/crm.js?v=20260830g'),
    ]);
    const crm = createCrmPanel({ $, api, admSkeleton, admEmpty });
