...
import { login, api, getToken } from './auth.js?v=20260804b';
import { esc, safeUrl, money, wireTablist, rovingTabindex, linkTabsToPanels } from './util.js?v=20260804b';
import { editKey } from './admin/edits.js?v=20260804b';
import { createFeatureLoader } from './admin/feature-loader.js?v=20260804b';
import { applyCapabilityUi, normalizeStaffContext, staffRoleLabel } from './admin/permissions.js?v=20260804b';
import { renderChrome } from './main/chrome.js?v=20260804b';

const $ = (id) => document.getElementById(id);

// Admin owns its staff-only runtime, but still needs shared site navigation.
renderChrome({ authModule: "/js/auth.js?v=20260804b", resolveSession: true });
...
function wireReports() {
  void import('./admin/stripe.js?v=20260804b').then(({ wireStripePayouts, renderStripePayouts }) => {
    wireStripePayouts();
...
const featureLoader = createFeatureLoader({
  analytics: async () => {
    const [{ createTrafficRenderer }, { createSeoAudit }] = await Promise.all([
      import('./admin/traffic.js?v=20260804b'),
      import('./admin/seo.js?v=20260804b'),
    ]);
  ...
  integrations: async () => {
    const { connectQbo, disconnectQbo, renderQboStatus, runQboSync } = await import('./admin/qbo.js?v=20260804b');
    const { renderShipStationStatus, wireShipStationStatus } = await import('./admin/shipstation.js?v=20260804b');
    const { renderStripeStatus } = await import('./admin/stripe.js?v=20260804b');
    const { renderIntegrationHealth, wireIntegrationHealth } = await import('./admin/integration-health.js?v=20260804b');
    return {
      wire() {
  ...
  orders: async () => {
    const { ORDER_STATUSES, createOrdersTab } = await import('./admin/orders.js?v=20260804b');
  ...
  companies: async () => {
    const [{ createCompaniesTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/companies.js?v=20260804b'),
      import('./admin/crm.js?v=20260804b'),
    ]);
  ...
  products: async () => {
    const [
      ...
      import('./admin/products.js?v=20260804b'),
      import('./admin/pricing.js?v=20260804b'),
      import('./admin/inventory.js?v=20260804b'),
      import('./admin/coupons.js?v=20260804b'),
    ]);
  ...
  content: async () => {
    const { createContentTab } = await import('./admin/content.js?v=20260804b');
  ...
  support: async () => {
    const { createThreadsTab } = await import('./admin/threads.js?v=20260804b');
  ...
  quotes: async () => {
    const { createQuotesTab } = await import('./admin/quotes.js?v=20260804b');
  ...
  reviews: async () => {
    const { createReviewsTab } = await import('./admin/reviews.js?v=20260804b');
    const {
      renderReviews,
      wireReviews,
  ...
  newsletter: async () => {
    const [{ createNewsletterTab }, { createOffersTab }] = await Promise.all([
      import('./admin/newsletter.js?v=20260804b'),
      import('./admin/offers.js?v=20260804b'),
    ]);
  ...
  crm: async () => {
    const [{ createCrmWorkspace }, { createCrmPanel }] = await Promise.all([
      import('./admin/crm-workspace.js?v=20260804b'),
      import('./admin/crm.js?v=20260804b'),
    ]);
    const crm = createCrmPanel({ $, api, admSkeleton, admEmpty });
