...
import { login, logout, api, getToken } from './auth.js?v=20260724e';
import { esc, safeUrl, money, wireTablist, rovingTabindex, linkTabsToPanels } from './util.js?v=20260724e';
import { editKey } from './admin/edits.js?v=20260724e';
import { createFeatureLoader } from './admin/feature-loader.js?v=20260724e';
import { applyCapabilityUi, normalizeStaffContext, staffRoleLabel } from './admin/permissions.js?v=20260724e';
import { renderChrome } from './main/chrome.js?v=20260724e';

const $ = (id) => document.getElementById(id);

// Admin owns its staff-only runtime, but still needs shared site navigation.
renderChrome({ authModule: "/js/auth.js?v=20260724e" });
...
const featureLoader = createFeatureLoader({
  analytics: async () => {
    const [{ createTrafficRenderer }, { createSeoAudit }] = await Promise.all([
      import('./admin/traffic.js?v=20260724e'),
      import('./admin/seo.js?v=20260724e'),
    ]);
  ...
  integrations: async () => {
    const { connectQbo, disconnectQbo, renderQboStatus, runQboSync } = await import('./admin/qbo.js?v=20260724e');
    return {
      wire() {
  ...
  orders: async () => {
    const { ORDER_STATUSES, createOrdersTab } = await import('./admin/orders.js?v=20260724e');
  ...
  companies: async () => {
    const [{ createCompaniesTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/companies.js?v=20260724e'),
      import('./admin/crm.js?v=20260724e'),
    ]);
  ...
  products: async () => {
    const [
      ...
      import('./admin/products.js?v=20260724e'),
      import('./admin/pricing.js?v=20260724e'),
      import('./admin/inventory.js?v=20260724e'),
      import('./admin/coupons.js?v=20260724e'),
    ]);
  ...
  content: async () => {
    const { createContentTab } = await import('./admin/content.js?v=20260724e');
  ...
  support: async () => {
    const { createThreadsTab } = await import('./admin/threads.js?v=20260724e');
  ...
  quotes: async () => {
    const { createQuotesTab } = await import('./admin/quotes.js?v=20260724e');
  ...
  reviews: async () => {
    const { createReviewsTab } = await import('./admin/reviews.js?v=20260724e');
    const {
      renderReviews,
      wireReviews,
  ...
  newsletter: async () => {
    const { createNewsletterTab } = await import('./admin/newsletter.js?v=20260724e');
  ...
  crm: async () => {
    const [{ createCrmWorkspace }, { createOffersTab }, { createCrmPanel }] = await Promise.all([
      import('./admin/crm-workspace.js?v=20260724e'),
      import('./admin/offers.js?v=20260724e'),
      import('./admin/crm.js?v=20260724e'),
    ]);
    const crm = createCrmPanel({ $, api, admSkeleton, admEmpty });
