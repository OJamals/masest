...
import { me, api, getToken } from './auth.js?v=20260912a';
      ...
      <div class="biz-reg-grid">
        ...
        <div class="biz-reg-full biz-cert-upload">
          ...
          <div class="biz-cert-row">
            <label class="btn btn-secondary btn-sm biz-cert-pick">Choose file<input id="resaleCertFile" name="resale_certificate" type="file" accept="application/pdf,image/png,image/jpeg,image/webp" hidden></label>
            <span id="certState" class="muted"></span>
          </div>
          <p class="biz-status" id="certStatus" role="status" aria-live="polite"></p>
        </div>` : ''}
        <label class="biz-check-label biz-reg-full"><input id="taxExempt" name="tax_exempt" type="checkbox" ${c.tax_exempt ? 'checked' : ''}> <span>We are tax-exempt and will provide a resale/exemption certificate.</span></label>
      </div>
    </details>`;
}
