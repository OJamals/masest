...
const heldProductSlugs = new Set(
  catalog.products
    .filter((product) => product.public_visible === false)
...
for (const variant of catalog.product_variants) {
  if (heldProductSlugs.has(variant.product_slug)) {
    variant.public_visible = false;
  }
...
const checkoutReadyVariants = activeVariants.filter((v) => v.public_visible !== false);
...
await writeFile(
  here('functions/_lib/product-publication.generated.js'),
  productPublicationModule(catalog),
);

const pendingProfiles = catalog.product_variants.filter((variant) => (
  variant.activation_blocker === 'shipping_package_profile_missing'
)).length;
console.log(`catalog: ${catalog.products.length} products, ${catalog.product_variants.length} variants, ${checkoutReadyVariants.length} checkout-ready, ${pendingProfiles} awaiting parcel profiles, ${(catalog.services || []).length + (catalog.service_packages || []).length} services`);
...
function publicActive(row) {
  return Boolean(row.active) && row.public_visible !== false;
}

function productPublicationModule(catalogData) {
  const heldProductSkus = catalogData.products
    .filter((product) => product.public_visible === false)
    .map((product) => product.slug)
    .sort();
  const heldProductSet = new Set(heldProductSkus);
  const heldReferences = [
    ...heldProductSkus,
    ...catalogData.product_variants
      .filter((variant) => heldProductSet.has(variant.product_slug))
      .map((variant) => variant.sku),
  ].map((sku) => sku.toLowerCase()).sort();
  return `// Generated by tools/build-catalog.mjs from data/catalog.seed.json. DO NOT edit by hand.\n`
    + `const HELD_PRODUCT_REFERENCES = new Set(${JSON.stringify(heldReferences)});\n\n`
