...
function variantsSql(variants) {
  ...
  const retailRows = variants
    .filter((v) => v.retail_price != null)
    .map((v) => `(${[sqlStr(v.sku), `'retail'`, Number(v.retail_price), sqlStr(v.currency || 'usd')].join(',')})`);
  return `-- MASEST product variants - generated by tools/build-catalog.mjs from data/catalog.seed.json. DO NOT edit by hand.\n`
    ...
    + `    delete from public.price_tiers where vsku not in (${currentVskus});\n`
    + `  end if;\n`
    + `end $$;\n\n`
