...
const migrations = [
  ...
  'migrate-durable-support-message-effects-2026-09-05.sql',
  ...
  'migrate-support-ticket-live-cutover-2026-09-07.sql',
...
const ids = {
  ...
  staff: '11000000-0000-4000-8000-000000000041',
  ...
  quotePaused: '40000000-0000-4000-8000-000000000043',
...
create table public.profiles (
  ...
  full_name text, staff_role text, is_staff boolean not null default false, notify_admin_support_requests boolean default true,
...
async function waitFor(client, text, params, predicate, message) {
...
async function main() {
  ...
  try {
    ...
    await apply(client, migrations[2]);

    // Reinstalling the real trigger proves its DDL lock drains a transaction that
    // predates trigger installation. The wait is observed in pg_stat_activity,
    // rather than inferred from elapsed time.
    peer = new Client({ ...cluster.clientConfig, application_name: 'fence-old-writer' });
    const installer = new Client({ ...cluster.clientConfig, application_name: 'fence-installer' });
    await peer.connect();
    await installer.connect();
    await peer.query('begin');
    await peer.query('lock table public.messages in row exclusive mode');
    let installSettled = false;
    const installing = apply(installer, migrations[2]).then(() => { installSettled = true; });
    await waitFor(client,
      `select wait_event_type, wait_event from pg_stat_activity where application_name='fence-installer'`, [],
      (rows) => rows.some((row) => row.wait_event_type === 'Lock'),
      'fence trigger installation did not observably drain the pre-trigger writer');
    assert.equal(installSettled, false);
    await peer.query('commit');
    await installing;

    // An accepted writer retains FOR SHARE until commit. Pause takes only the
    // singleton FOR UPDATE and therefore waits for that writer to drain.
    await peer.query('begin');
    await one(peer, 'select public.assert_support_writes_enabled() ok');
    let pauseSettled = false;
    const pausing = one(installer,
      `select public.set_support_ingress_accepting(1,false,'local cutover proof') result`)
      .then((value) => { pauseSettled = true; return value; });
    await waitFor(client,
      `select wait_event_type from pg_stat_activity where application_name='fence-installer'`, [],
      (rows) => rows.some((row) => row.wait_event_type === 'Lock'),
      'pause did not observably wait for the accepted writer');
    assert.equal(pauseSettled, false);
    await peer.query('commit');
    const paused = await pausing;
    assert.equal(paused.result.accepting, false);
    await assert.rejects(peer.query('select public.assert_support_writes_enabled()'), /support_writes_paused/);
    const resumed = await one(installer,
      `select public.set_support_ingress_accepting(2,true,null) result`);
    assert.equal(resumed.result.accepting, true);

    // A transaction with a stale RR snapshot must not accept after pause.
    await peer.query('begin isolation level repeatable read');
    await peer.query('select accepting from public.support_ingress_control');
    await one(installer, `select public.set_support_ingress_accepting(3,false,'stale snapshot proof') result`);
    await assert.rejects(peer.query('select public.assert_support_writes_enabled()'),
      /could not serialize|support_writes_paused/i);
    await peer.query('rollback');
    await one(installer, `select public.set_support_ingress_accepting(4,true,null) result`);
    await installer.end();
    await peer.end();
    peer = null;
    await client.query(`insert into public.companies(id, name) values ($1, 'Proof Co')`, [ids.company]);
    await client.query(`insert into public.profiles(id, company_id, full_name) values ($1,$2,'Buyer')`, [ids.buyer, ids.company]);
    await client.query(`insert into public.profiles(id, full_name, staff_role, is_staff) values ($1,'Staff','support',true)`, [ids.staff]);
    ...
    for (const migration of migrations.slice(3, 5)) await apply(client, migration);
    const legacyTicket = await one(client, 'select id from public.support_tickets where thread_id=$1', [ids.thread]);
    const historical = await one(client, `
      insert into public.messages(thread_id,ticket_id,company_id,user_id,sender_role,body,order_id,source)
      values ($1,$2,$3,$4,'buyer','Historical pre-cutover message',$5,'dashboard')
      returning jsonb_build_object('id',id,'ticket_id',ticket_id) result
    `, [ids.thread, legacyTicket.id, ids.company, ids.buyer, ids.order]);

    const before = await one(client, `select count(*)::int as count from public.integration_events where provider = 'masest'`);
    assert.equal(before.count, 1, 'predecessor producer did not create its durable intent');
    const predecessor = await one(client, `
      select event.provider_event_type, effect.effect_key, effect.effect_type, effect.status
        from public.integration_effects effect
        join public.integration_events event on event.id = effect.event_id
       where event.provider = 'masest'
    `);
    assert.deepEqual(predecessor, {
      provider_event_type: 'support_message_created', effect_key: 'support-email',
      effect_type: 'support_message_email', status: 'pending',
    }, 'predecessor fixture did not create the expected nonterminal legacy shape');
    peer = new Client({ ...cluster.clientConfig, application_name: 'fence-legacy-writer' });
    const legacyPauser = new Client({ ...cluster.clientConfig, application_name: 'fence-legacy-pauser' });
    await peer.connect();
    await legacyPauser.connect();
    await peer.query('begin');
    await peer.query(`insert into public.messages(thread_id,ticket_id,company_id,user_id,sender_role,body,source)
      values ($1,$2,$3,$4,'buyer','legacy drain proof','dashboard')`, [ids.thread, legacyTicket.id, ids.company, ids.buyer]);
    const legacyPause = one(legacyPauser, `select public.set_support_ingress_accepting(5,false,'legacy writer drain') result`);
    await waitFor(client,
      `select wait_event_type from pg_stat_activity where application_name='fence-legacy-pauser'`, [],
      (rows) => rows.some((row) => row.wait_event_type === 'Lock'),
      'pause did not wait for an old direct-insert/legacy-trigger transaction');
    await peer.query('rollback');
    await legacyPause;
    await one(legacyPauser, `select public.set_support_ingress_accepting(6,true,null) result`);
    await peer.end();
    await legacyPauser.end();
    peer = null;
    const migrationPause = await one(client,
      `select public.set_support_ingress_accepting(7,false,'migration sequence proof') result`);
    assert.equal(migrationPause.result.accepting, false);
    await assert.rejects(
      apply(client, migrations[5]),
      /support_message_legacy_effects_nonterminal/,
      'live cutover accepted undrained predecessor work',
    );
    await client.query(`
      update public.integration_effects
         set status = 'completed', completed_at = now(), updated_at = now()
       where effect_type = 'support_message_email'
    `);
    const legacyBefore = await one(client, `
      select count(*)::int as count,
             md5(string_agg(row_to_json(effect)::text, '' order by effect.id)) as digest
        from public.integration_effects effect
       where effect.effect_type = 'support_message_email'
    `);
    await apply(client, migrations[5]);
    await apply(client, migrations[5]);
    await apply(client, migrations[6]);
    await apply(client, migrations[7]);
    ...
    const migrationResume = await one(client,
      `select public.set_support_ingress_accepting(8,true,null) result`);
    assert.equal(migrationResume.result.accepting, true);
    await peer?.end().catch(() => {});
    peer = new Client(cluster.clientConfig);
    await peer.connect();
    await peer.query('begin isolation level serializable');
    await peer.query('select accepting from public.support_ingress_control');
    await one(client, `select public.set_support_ingress_accepting(9,false,'serializable snapshot proof') result`);
    await assert.rejects(peer.query('select public.assert_support_writes_enabled()'),
      /could not serialize|support_writes_paused/i);
    await peer.query('rollback');
    await one(client, `select public.set_support_ingress_accepting(10,true,null) result`);
    await peer.end();
    peer = null;
    assert.equal((await one(client, 'select public.assert_email_effects_ready() ready')).ready, true);
    await client.query('alter table public.messages alter column ticket_id drop not null');
    await assert.rejects(client.query('select public.assert_email_effects_ready()'), /durable_email_effects_not_ready/);
    await client.query('alter table public.messages alter column ticket_id set not null');
    await client.query('alter table public.messages rename column ticket_id to ticket_id_proof_missing');
    await assert.rejects(client.query('select public.assert_email_effects_ready()'), /durable_email_effects_not_ready/);
    await client.query('alter table public.messages rename column ticket_id_proof_missing to ticket_id');
    await client.query('alter table public.messages disable trigger messages_support_message_email_after_insert');
    await assert.rejects(client.query('select public.assert_email_effects_ready()'), /durable_email_effects_not_ready/);
    await client.query('alter table public.messages enable replica trigger messages_support_message_email_after_insert');
    await assert.rejects(client.query('select public.assert_email_effects_ready()'), /durable_email_effects_not_ready/);
    await client.query('drop trigger messages_support_message_email_after_insert on public.messages');
    await client.query(`create trigger messages_support_message_email_after_insert after insert on public.messages
    ...
    peer = new Client({ ...cluster.clientConfig, application_name: 'fence-rpc-writer' });
    const pauser = new Client({ ...cluster.clientConfig, application_name: 'fence-rpc-pauser' });
    await peer.connect();
    await pauser.connect();
    await peer.query('begin');
    await one(peer, `select public.append_support_message($1,$2,'buyer','drain proof',$3,'dashboard',false,null,$2,$4,$5,'Drain','general',false,2) result`, [ids.company, ids.buyer, ids.order, ids.thread, historical.result.ticket_id]);
    const drainingPause = one(pauser, `select public.set_support_ingress_accepting(11,false,'actual writer drain') result`);
    await waitFor(client,
      `select wait_event_type from pg_stat_activity where application_name='fence-rpc-pauser'`, [],
      (rows) => rows.some((row) => row.wait_event_type === 'Lock'),
      'pause did not wait for an actual append_support_message transaction');
    await peer.query('rollback');
    await drainingPause;
    await one(pauser, `select public.set_support_ingress_accepting(12,true,null) result`);
    await peer.end();
    await pauser.end();
    peer = null;
    await client.query(`insert into public.quotes(id,payload) values ($1,jsonb_build_object('company_id',$2::text))`, [ids.quotePaused, ids.company]);
    const pausedQuoteEvent = await one(client, `select public.ingest_integration_event('masest','proof','quote-paused','quote.ready',$1,now(),null,encode(extensions.digest('quote-paused','sha256'),'hex'),'{}',jsonb_build_array(jsonb_build_object('effect_key','quote-message','effect_type','quote_message','aggregate_type','quote','aggregate_id',$1::text,'payload',jsonb_build_object('company_id',$2::text,'quote_id',$1::text)))) id`, [ids.quotePaused, ids.company]);
    const pausedClaims = await client.query(`select * from public.claim_integration_effects('proof-paused-quote',25,60)`);
    const pausedQuoteEffect = pausedClaims.rows.find((row) => row.event_id === pausedQuoteEvent.id);
    assert.ok(pausedQuoteEffect);
    const beforePausedWriters = await fixtureState(client);
    await one(client, `select public.set_support_ingress_accepting(13,false,'writer rollback proof') result`);
    await assert.rejects(client.query(`select public.append_support_message($1,$2,'buyer','paused',$3,'dashboard',false,null,$2,$4,$5,'Paused','general',false,2)`, [ids.company, ids.buyer, ids.order, ids.thread, historical.result.ticket_id]), /support_writes_paused/);
    await assert.rejects(client.query(`select public.upsert_email_inbound_message($1,$2,'<paused@example.test>','paused','buyer',null,$3,null,$4,$5)`, [ids.company, ids.buyer, ids.order, ids.thread, historical.result.ticket_id]), /support_writes_paused/);
    await assert.rejects(client.query(`select public.create_order_support_request($1,'cancel','paused','[]'::jsonb,$2,'buyer@example.test','paused',2)`, [ids.order, ids.buyer]), /support_writes_paused/);
    await assert.rejects(client.query(`select public.update_support_ticket($1,1,$2,'resolved',null,null,null,false)`, [historical.result.ticket_id, ids.staff]), /support_writes_paused/);
    await assert.rejects(client.query(`select public.deliver_quote_message_effect($1,'proof-paused-quote')`, [pausedQuoteEffect.id]), /support_writes_paused/);
    assert.deepEqual(await fixtureState(client), beforePausedWriters, 'paused support writers changed upstream state');
    await client.query(`insert into public.quotes(id,payload) values ($1,'{}')`, [ids.quoteRollback]);
    await client.query(`delete from public.quotes where id=$1`, [ids.quoteRollback]);
    await assert.rejects(client.query(`select public.set_support_ingress_accepting(13,true,null)`), /support_ingress_generation_conflict/);
    await client.query('begin');
    await client.query('delete from public.support_ingress_control');
    await assert.rejects(client.query('select public.assert_support_writes_enabled()'), /support_writes_paused/);
    await client.query('rollback');
    peer = new Client(cluster.clientConfig);
    await peer.connect();
    await peer.query('set role service_role');
    await assert.rejects(peer.query('update public.support_ingress_control set accepting=true'), /permission denied/i);
    await assert.rejects(peer.query(`select public.set_support_ingress_accepting(14,true,null)`), /permission denied/i);
    await peer.end();
    peer = null;
    await one(client, `select public.set_support_ingress_accepting(14,true,null) result`);
    const ticketVersion = await one(client, 'select version from public.support_tickets where id=$1', [historical.result.ticket_id]);
    peer = new Client({ ...cluster.clientConfig, application_name: 'fence-status-writer' });
    const statusPauser = new Client({ ...cluster.clientConfig, application_name: 'fence-status-pauser' });
    await peer.connect();
    await statusPauser.connect();
    await peer.query('begin');
    await one(peer, `select public.update_support_ticket($1,$2,$3,null,'high',null,null,false) result`, [historical.result.ticket_id, ticketVersion.version, ids.staff]);
    const statusPause = one(statusPauser, `select public.set_support_ingress_accepting(15,false,'status writer drain') result`);
    await waitFor(client,
      `select wait_event_type from pg_stat_activity where application_name='fence-status-pauser'`, [],
      (rows) => rows.some((row) => row.wait_event_type === 'Lock'),
      'pause did not wait for an actual update_support_ticket transaction');
    await peer.query('rollback');
    await statusPause;
    await one(statusPauser, `select public.set_support_ingress_accepting(16,true,null) result`);
    await peer.end();
    await statusPauser.end();
    peer = null;
    const afterMigration = await one(client, `select count(*)::int as count from public.integration_events where provider = 'masest' and provider_event_type in ('support_message_created','support.message.created')`);
    assert.equal(afterMigration.count, 1, 'live cutover synthesized historical message delivery');
    const legacyAfter = await one(client, `
      select count(*)::int as count,
             md5(string_agg(row_to_json(effect)::text, '' order by effect.id)) as digest
        from public.integration_effects effect
       where effect.effect_type = 'support_message_email'
    `);
    assert.deepEqual(legacyAfter, legacyBefore, 'live cutover changed predecessor ledger evidence');
    ...
    const effects = await client.query(`select effect.* from public.integration_effects effect join public.integration_events event on event.id=effect.event_id where event.provider='masest' and event.provider_event_type='support.message.created' and effect.effect_type='support_message_email'`);
    ...
    console.log(JSON.stringify({ proof: 'support-ticket-live-cutover',
      ok: true,
