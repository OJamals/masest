...
test.beforeAll(async () => {
  ...
  await waitForHttpServer(`${BASE_URL}/cart.html`);
});
...
test("cart loads one canonical auth module instance", async ({ page }) => {
  const authRequests = [];
  const duplicateClientWarnings = [];
  page.on("request", (request) => {
    if (new URL(request.url()).pathname === "/js/auth.js") authRequests.push(request.url());
  });
  page.on("console", (message) => {
    if (message.text().includes("Multiple GoTrueClient instances detected")) {
      duplicateClientWarnings.push(message.text());
    }
  });
  ...
  expect(duplicateClientWarnings).toEqual([]);
});
...
test("cart shows server catalog prices and estimated subtotal", async ({ page }) => {
  ...
  await expect(page.getByText("Estimated subtotal")).toBeVisible();
