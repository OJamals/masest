...
export async function onRequestGet({ request, env }) {
  ...
  let orderQuery = sb
    ...
    .eq('id', id);
  orderQuery = companyId ? orderQuery.eq('company_id', companyId) : orderQuery.eq('user_id', user.id);
...
export async function handleAccountOrderPost({ request, env }, dependencies = {}) {
  const getCompany = dependencies.requireCommerceUser || dependencies.requireCompany || requireCommerceUser;
  const parseBody = dependencies.readBody || readBody;
  const ctx = await getCompany(request, env);
  if (ctx.error) return ctx.error;
  const { companyId, user, sb } = ctx;
  ...
  const { data: variants, error: catalogError } = skus.length
    ? await sb.from('product_variants').select('vsku,price,active').in('vsku', skus)
    : { data: [], error: null };
  if (catalogError) return json(503, { error: 'catalog_unavailable', retryable: true });
  const map = {};
...
export async function onRequestPost(context) {
  return handleAccountOrderPost(context);
}
