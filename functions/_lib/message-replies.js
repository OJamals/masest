...
export async function messageReplyAddress(env, companyId) {
  ...
  const token = (await signature(`message-reply:${id}`, secret)).slice(0, 24);
...
export async function companyIdFromReplyAddress(env, recipients) {
  ...
  const matchRe = new RegExp(`^reply\\+(${UUID_RE.source.slice(1, -1)})\\.([a-f0-9]{24})@${escapedDomain}$`, 'i');
  for (const recipient of values) {
    const match = String(recipient || '').trim().toLowerCase().match(matchRe);
    if (!match) continue;
    const [, id, token] = match;
    const expected = (await signature(`message-reply:${id}`, secret)).slice(0, 24);
