...
export function publicPricingPayload({
  ...
  return {
    ...
    variants: variants.map((variant) => ({
      ...
      product_name: variant.marketing_name || variant.products?.name || variant.product_name || variant.product_sku,
      label: variant.label,
      gallons: Number(variant.gallons ?? variant.size_gal ?? 0),
      market: variant.market || "industrial",
      package_kind: variant.package_kind || "unit",
      units_per_case: Number(variant.units_per_case || 1),
      unit_vsku: variant.unit_vsku || null,
      requires_quote: variant.requires_quote === true,
      pricing_source_version: variant.pricing_source_version || null,
