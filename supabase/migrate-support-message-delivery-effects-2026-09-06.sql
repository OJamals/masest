...
begin
  if to_regclass('public.integration_events') is null
     ...
     or to_regclass('public.support_message_email_envelopes') is null
     or to_regclass('public.support_ticket_routing_contract') is null
     or to_regprocedure('public.claim_support_message_email_effect(uuid,text,integer)') is null
     or to_regprocedure('public.store_support_message_email_envelope(uuid,text,jsonb)') is null
     or not exists (select 1 from pg_attribute where attrelid = 'public.messages'::regclass
          and attname = 'ticket_id' and attnotnull and not attisdropped)
     or not exists (select 1 from public.support_ticket_routing_contract
          where singleton = true and version in (1, 2))
     or not exists (select 1 from pg_trigger
          where tgname = 'messages_support_message_email_after_insert'
            and tgrelid = 'public.messages'::regclass
            and tgfoid = 'public.enqueue_support_message_email_effect()'::regprocedure
            and tgenabled in ('O', 'A'))
     or exists (select 1 from pg_trigger
          where tgname = 'messages_support_email_effect'
            and tgrelid = 'public.messages'::regclass and tgenabled <> 'D')
     or not exists (select 1 from pg_trigger
          where tgname = 'quotes_intake_email_effect'
