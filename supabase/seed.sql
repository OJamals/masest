-- MASEST product parents — generated by tools/build-catalog.mjs from data/catalog.seed.json. DO NOT edit by hand.
-- Run AFTER schema.sql, then variants_seed.sql. Prices are null on parents (variant-priced).
-- mode=buy → cart/checkout UI; mode=quote → quote form only.
insert into public.products (sku, name, group_key, hmis, mode, hazmat, taxable, price, sort) values
  ('hcr', 'VertKleen HCR', 'degrease', '0-0-0', 'buy', false, true, null, 1),
  ('cr', 'VertKleen CR', 'degrease', '0-0-0', 'buy', false, true, null, 2),
  ('descaler', 'VertKleen Descaler', 'degrease', '0-0-0', 'buy', false, true, null, 3),
  ('cr-hd', 'VertKleen CR HD', 'degrease', '0-0-0', 'buy', false, true, null, 4),
  ('cr-hd-low-foam', 'VertKleen CR HD Low Foam', 'degrease', '0-0-0', 'buy', false, true, null, 5),
  ('neutral', 'VertKleen Neutral', 'degrease', '0-0-0', 'buy', false, true, null, 6),
  ('multiwash', 'VertKleen MultiWash', 'degrease', '0-0-0', 'buy', false, true, null, 7),
  ('torque', 'VertKleen Torque', 'degrease', '0-0-0', 'buy', false, true, null, 8),
  ('alumibrite', 'VertKleen AlumiBrite', 'degrease', '0-0-0', 'buy', false, true, null, 9),
  ('purgo', 'VertKleen Purgo', 'water', '0-0-0', 'buy', false, true, null, 10),
  ('lam3', 'VertKleen LAM3', 'water', '0-0-0', 'buy', false, true, null, 11),
  ('watersafe60', 'WaterSafe60', 'water', '0-0-0', 'quote', false, true, null, 12),
  ('cr2', 'VertKleen CR2', 'water', '0-0-0', 'quote', false, true, null, 13),
  ('sar', 'VertKleen SAR', 'water', '0-0-0', 'quote', false, true, null, 14),
  ('pg100', 'PG inhibited 100% concentrate', 'glycol', '0-0-0', 'quote', false, true, null, 15),
  ('pg50', 'PG inhibited 50% RTU', 'glycol', '0-0-0', 'quote', false, true, null, 16),
  ('eg100', 'EG inhibited 100% concentrate', 'glycol', '0-0-0', 'quote', false, true, null, 17),
  ('eg50', 'EG inhibited 50% RTU', 'glycol', '0-0-0', 'quote', false, true, null, 18),
  ('egu96', 'EG uninhibited 96% concentrate', 'glycol', '0-0-0', 'quote', false, true, null, 19),
  ('eg5050', 'EG 50/50', 'glycol', '0-0-0', 'quote', false, true, null, 20)
on conflict (sku) do update set
  name = excluded.name,
  group_key = excluded.group_key,
  hmis = excluded.hmis,
  mode = excluded.mode,
  hazmat = excluded.hazmat,
  taxable = excluded.taxable,
  sort = excluded.sort,
  updated_at = now();
  -- price intentionally NOT updated, so owner-set prices survive a re-run.
